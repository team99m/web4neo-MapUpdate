...
import type { IssueCategory, ReportStep } from '../types'
...
export default function ReportPage() {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      const map = L.map(mapContainerRef.current, { center: [13.7563, 100.5018], zoom: 13, zoomControl: false })
    ...
    try {
      ...
      for (const photo of photos) {
        ...
        if (upErr) {
          ...
          // Don't stop the whole submission, but alert the user or log it
          continue
        }
  ...
  if (authLoading) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>Loading…</div>
